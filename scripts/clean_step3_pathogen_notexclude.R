...
length(unique(ss01$ICU_ID_from_datasource))    #208
...
#only get the Bacteremia groups
clinical_marker_scource <- clinical_marker_scource %>%
  dplyr::filter(Microbe_groups == "Bacteremia")
...
#data3 <- data %>%
#select(ICU_ID_from_datasource, FinalGroup_pathegon, microbe_result) %>%
#filter(FinalGroup_pathegon == "Mixed_pathegon")
