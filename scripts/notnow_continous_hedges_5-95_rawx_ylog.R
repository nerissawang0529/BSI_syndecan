...
get_slope_vector <- function(marker) {
  # 只用中间90%的 Syndecan-1 做拟合
  synd_bounds <- quantile(merged_data$Syndecan1, c(0.05, 0.95), na.rm = TRUE)
  data_trimmed <- merged_data %>%
    filter(Syndecan1 >= synd_bounds[1], Syndecan1 <= synd_bounds[2])
  
  # 拟合 & 全域预测
  fit <- lm(reformulate("rcs(Syndecan1, 3)", paste0("log10(`", marker, "` + 1)")), data = data_trimmed)
...
make_heatmap <- function(marker_list, heatmap_name, col_fun = NULL) {
  ...
  Heatmap(
    ...
    heatmap_legend_param = list(title = "slope", title_gp = gpar(fontsize = 12, fontfamily = "sans"),
...
# === 导出 PDF（缩小宽度）===
pdf("figures/Figure_continous_hedgesg_5_95.pdf", width = 4, height = 12)
...
plot_single_marker <- function(marker, color = "black", y_lim = c(0, 7)) {
  ...
  # 计算 Syndecan-1 5–95% 分位范围
  synd_bounds <- quantile(df$Syndecan1, c(0.05, 0.95), na.rm = TRUE)
  df_trimmed <- df %>% filter(Syndecan1 >= synd_bounds[1], Syndecan1 <= synd_bounds[2])
  
  # 用中间90%的数据拟合
  fit <- lm(reformulate("rcs(Syndecan1, 3)", response = marker), data = df_trimmed)
  ...
  ggplot(df, aes(x = Syndecan1, y = .data[[marker]])) +
    ...
    theme(
      plot.title = element_text(size = 11, hjust = 0.5, family = "sans"),
...
final_plot <- wrap_plots(plot_list, ncol = ncol) +
  plot_annotation(
    ...
    theme = theme(
      plot.title = element_text(size = 14, face = "bold", family = "sans", hjust = 0),
...
# === 导出 SVG 图像 ===
ggsave("figures/FigureS_spline_markers_5_95.svg", final_plot, width = 15, height = 16)
