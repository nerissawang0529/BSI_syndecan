...
data$Syndecan_group <- cut(data$syndecan_original,
                           breaks = quantile(data$syndecan_original, probs = seq(0, 1, length = 4), na.rm = TRUE),
                           include.lowest = TRUE,
                           labels = c(1, 2, 3))  # Now using 4 groups


data2 <- data[,c("ICU_ID_from_datasource","Syndecan_group","APACHE_IV_Score.x","SOFAtot_new.x")]
data2 <- data2[!is.na(data2$SOFAtot_new), ]
#3717,5881,6658 has no sofa but has transcriptome
...
yi_1 <- yi
#yi_1$new_adjusted_pvalue <- p.adjust(yi$pvalue, method = "BH") #no need to adjusted again
