...
data$ANG_ratio <- data$Ang.2..26./data$Ang.1..64.

#delete the outliers for endothelial 
#4298 14507  1535
data <- data %>% 
  filter(!ICU_ID_from_datasource %in% c(4298, 14507, 1535))
...
data_table1 <- data[,c("ICU_ID_from_datasource","Syndecan.1.CD138..29.","age_yrs","gender","BMI","Microbe_groups","COPD","Chronic_heart_failure","Past_myocardial_infarction","Cerebrovascular_disease","mneoplasm","ckd","diabetes","Platelets_value_1","WBC_2_1","Creatinine_value_1","Blood_Urea_Nitrogen_value_1","Bilirubin","SOFAtot","ICUA_no_complication","ICUA_infection","MEWS_score","Charlson_with_age","APACHE_IV_Score.x","length_of_stay",
                                                           "mort30","mort90","Shock_on_admis","ICUA_no_complication","ICUA_infection","Death_in_hospital.x","death_in_ICU_correct","Klebsiella","S.aureus","CoNS","Staph_unknown","Enterococcus","E.coli","Streptococcus","microbenum","Syndecan_group","Microbe_groups",
                       "CX3CL1.Fractalkine..46.","Cystatin.C..75.","ESM.1..22.","Ang.1..64.","Ang.2..26.","ANG_ratio")]

#delete the "Viral", "Fungal"
## i want to make the table one only for bacteremia and non-infectious
#this is an optional step, if i only want the bacteremia and non-infectious
data_table1 <- data_table1[!(data_table1$Microbe_groups %in% c("Viral", "Fungal")), ]
...
allvars <- c("age_yrs","gender","BMI",
             ...
             "length_of_stay","Death_in_hospital.x","death_in_ICU_correct","mort30","mort90","Microbe_groups")
...
export_file_name <- "table1_Stratified_by_bacteria_non_infection.csv" 
...
##corrected for markers####
#####corrected for severity####
#checked if linearity with sofa/APACHE_IV_Score.x
linear_model_mic_non <- lm(log(ANG_ratio) ~ SOFAtot, data = data_table1)
spline_model_mic_non <- lm(log(ANG_ratio) ~ rcs(SOFAtot, 3), data = data_table1)

# Compare models using ANOVA
model_comparison_mews <- anova(linear_model_mic_non, spline_model_mic_non)
...
linear_model_correct_bac_non <- lm(log(Syndecan.1.CD138..29.) ~ SOFAtot + APACHE_IV_Score.x + Microbe_groups, data = data_table1)
# Print the summary of the model
anova(linear_model_correct_bac_non)
#####




#data %>% select(c(Gram_positive, Gram_negative,microbe_result,Microbe_groups)) %>% View()

####Gram-positive vs Gram-nagetive####
#when one patient is Gram_negative and Gram_positive at the same time, then put them in both group
# Create category variable

data_gram_positive<- data[data$Gram_negative == 1 & data$Gram_positive == 1, ] 
data_gram_positive$Gram_positive <- 0
data_2 <- data
data_2[data_2$Gram_negative == 1 & data_2$Gram_positive == 1, "Gram_negative"] <- 0
data_2 <- rbind(data_gram_positive,data_2) 
...
#table
#pick column what i want
data_table1_gram <- data_2[,c("ICU_ID_from_datasource","Syndecan.1.CD138..29.","age_yrs","gender","BMI","Microbe_groups","COPD","Chronic_heart_failure","Past_myocardial_infarction","Cerebrovascular_disease","mneoplasm","ckd","diabetes","Platelets_value_1","WBC_2_1","Creatinine_value_1","Blood_Urea_Nitrogen_value_1","Bilirubin","SOFAtot","ICUA_no_complication","ICUA_infection","MEWS_score","Charlson_with_age","APACHE_IV_Score.x","length_of_stay",
                       "mort30","mort90","Shock_on_admis","ICUA_no_complication","ICUA_infection","Death_in_hospital.x","death_in_ICU_correct","Klebsiella","S.aureus","CoNS","Staph_unknown","Enterococcus","E.coli","Streptococcus","microbenum","Syndecan_group","Gram_group",
                       "CX3CL1.Fractalkine..46.","Cystatin.C..75.","ESM.1..22.","Ang.1..64.","Ang.2..26.","ANG_ratio")]
##  
allvars <- c("age_yrs","gender","BMI",
             "COPD","Chronic_heart_failure","Past_myocardial_infarction","Cerebrovascular_disease","mneoplasm","ckd","diabetes","ICUA_no_complication",
             ...
             "ICUA_infection","Microbe_groups","Klebsiella","S.aureus","CoNS","Staph_unknown","Enterococcus","E.coli","Streptococcus",
             "SOFAtot","MEWS_score","APACHE_IV_Score.x","Shock_on_admis",
             "length_of_stay","Death_in_hospital.x","death_in_ICU_correct","mort30","mort90","Gram_group")

catvars <- c("gender","COPD",	"Chronic_heart_failure","Past_myocardial_infarction","Cerebrovascular_disease",	"mneoplasm","ckd","diabetes","ICUA_no_complication","ICUA_infection","Gram_group","Shock_on_admis","Death_in_hospital.x","death_in_ICU_correct","mort30","mort90","Klebsiella","S.aureus","CoNS","Staph_unknown","Enterococcus","E.coli","Streptococcus")

nonnormal <- c("Syndecan.1.CD138..29.","CX3CL1.Fractalkine..46.","Cystatin.C..75.","ESM.1..22.","Ang.1..64.","Ang.2..26.","ANG_ratio","age_yrs","BMI","Platelets_value_1", "WBC_2_1","Creatinine_value_1", "Blood_Urea_Nitrogen_value_1","Bilirubin","SOFAtot","MEWS_score","APACHE_IV_Score.x","length_of_stay")

tab1     <- CreateTableOne(
  vars        = allvars, 
  data        = data_table1_gram, 
  strata = "Gram_group",
  factorVars  = catvars,
  test        = TRUE)
print(tab1, nonnormal = nonnormal, quote = FALSE, noSpaces = TRUE, smd = T, missing = T)
...
#export clinical_marker_unique data
destination_folder <- "Documents/BSI/R_code/original_data" 
export_file_name <- "table1_Stratified_by_Gram_group_duplcated.csv" 
write.csv(tab1_df, file.path(destination_folder, export_file_name), row.names = FALSE)
cat("File exported to:", file.path(destination_folder, export_file_name), "\n")
...
#####corrected for severity####
#checked if linearity with sofa/APACHE_IV_Score.x
linear_model_gram <- lm(log(ANG_ratio) ~ APACHE_IV_Score.x, data = data_table1_gram)
spline_model_gram <- lm(log(ANG_ratio) ~ rcs(APACHE_IV_Score.x, 3), data = data_table1_gram)
...
#the relationship is linearity. 
# Linear model with log(Syndecan) as the outcome and group, SOFAtot, and APACHE_IV_Score.x as predictors
linear_model_correct_gram <- lm(log(ANG_ratio) ~ SOFAtot + APACHE_IV_Score.x + Gram_group, data = data_table1_gram)
# Print the summary of the model
...
data_table1_seven <- data_bacteria[,c("ICU_ID_from_datasource","Syndecan.1.CD138..29.","age_yrs","gender","BMI","Microbe_groups","COPD","Chronic_heart_failure","Past_myocardial_infarction","Cerebrovascular_disease","mneoplasm","ckd","diabetes","Platelets_value_1","WBC_2_1","Creatinine_value_1","Blood_Urea_Nitrogen_value_1","Bilirubin","SOFAtot","ICUA_no_complication","ICUA_infection","MEWS_score","Charlson_with_age","APACHE_IV_Score.x","length_of_stay",
                              "mort30","mort90","Shock_on_admis","ICUA_no_complication","ICUA_infection","Death_in_hospital.x","death_in_ICU_correct","Klebsiella","S.aureus","CoNS","Staph_unknown","Enterococcus","E.coli","Streptococcus","microbenum","Syndecan_group","seven_group",
                              "CX3CL1.Fractalkine..46.","Cystatin.C..75.","ESM.1..22.","Ang.1..64.","Ang.2..26.","ANG_ratio")]
##  
allvars <- c("age_yrs","gender","BMI",
             "COPD","Chronic_heart_failure","Past_myocardial_infarction","Cerebrovascular_disease","mneoplasm","ckd","diabetes","ICUA_no_complication",
             ...
             "ICUA_infection","Microbe_groups","Klebsiella","S.aureus","CoNS","Staph_unknown","Enterococcus","E.coli","Streptococcus",
             "SOFAtot","MEWS_score","APACHE_IV_Score.x","Shock_on_admis",
             "length_of_stay","Death_in_hospital.x","death_in_ICU_correct","mort30","mort90","seven_group")

catvars <- c("gender","COPD",	"Chronic_heart_failure","Past_myocardial_infarction","Cerebrovascular_disease",	"mneoplasm","ckd","diabetes","ICUA_no_complication","ICUA_infection","seven_group","Shock_on_admis","Death_in_hospital.x","death_in_ICU_correct","mort30","mort90","Klebsiella","S.aureus","CoNS","Staph_unknown","Enterococcus","E.coli","Streptococcus")
...
#export clinical_marker_unique data
destination_folder <- "Documents/BSI/R_code/original_data" 
export_file_name <- "table1_Stratified_by_seven_group.csv" 
write.csv(tab1_df, file.path(destination_folder, export_file_name), row.names = FALSE)
cat("File exported to:", file.path(destination_folder, export_file_name), "\n")

#####corrected for severity####
#checked if linearity with sofa/APACHE_IV_Score.x
linear_model_seven <- lm(log(ANG_ratio) ~ APACHE_IV_Score.x, data = data_bacteria)
spline_model_seven <- lm(log(ANG_ratio) ~ rcs(APACHE_IV_Score.x, 3), data = data_bacteria)
...
#the relationship is linearity. 
# Linear model with log(Syndecan) as the outcome and group, SOFAtot, and APACHE_IV_Score.x as predictors
linear_model_correct_seven <- lm(log(Syndecan.1.CD138..29.) ~ SOFAtot + APACHE_IV_Score.x + seven_group, data = data_bacteria)
# Print the summary of the model
